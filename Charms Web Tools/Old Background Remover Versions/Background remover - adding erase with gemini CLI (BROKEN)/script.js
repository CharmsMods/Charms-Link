...
// NEW: Erase/Replace (Brush) Tool DOM Elements
const brushToolToggle = document.getElementById('brushToolToggle');
...
const brushModeReplace = document.getElementById('brushModeReplace');
...
// NEW: Erase/Replace (Brush) Tool Global Variables
let isBrushToolActive = false;
let brushShape = 'circle'; // 'circle' or 'square'
let brushMode = 'replace'; // 'replace' or 'erase'
let brushSize = 20;
...
function loadImage(event) {
    ...
    reader.onload = function (e) {
        ...
        originalImage.onload = function () {
            ...
            // NEW: Reset Brush Tool
            isBrushToolActive = false;
            brushToolToggle.checked = false;
            ...
            brushModeReplace.classList.add('active');
...
function applyFilter(forceUpdate = false) {
    if (isBrushToolActive) return; // Do not apply global filter if brush is active
...
// NEW: Brush/Erase Tool functions and listeners

/**
 * Disables or enables main controls based on whether the brush tool is active.
 */
function updateControlStates() {
    const controlsToDisable = [
        opacitySlider,
        toleranceToggle,
        toleranceStrengthSlider,
        invertSelectionToggle,
        antiAliasingToggle,
        smoothingFactorSlider,
        colorReplacementToggle,
        areaSelectionToggle,
        previewButton
    ];
    
    if (isBrushToolActive) {
        // Brush is active, disable other controls
        controlsToDisable.forEach(control => {
            control.disabled = true;
            control.parentElement.classList.add('disabled');
        });
        showMessage('Brush tool enabled. Click and drag on the canvas to paint.', 'info');
        if (isRealtimePreviewEnabled) applyFilter();
    } else {
        // Brush is inactive, enable other controls
        controlsToDisable.forEach(control => {
            control.disabled = false;
            control.parentElement.classList.remove('disabled');
        });
        if (originalImageData) applyFilter(true); // Redraw canvas with original settings
    }
}

/**
 * Draws on the canvas based on the brush settings.
 * @param {MouseEvent} e - The mouse event.
 */
function drawOnCanvas(e) {
    if (!isPainting || !originalImageData) return;
    ...
    for (let i = -radius; i <= radius; i++) {
        for (let j = -radius; j <= radius; j++) {
            ...
            if (pixelX < 0 || pixelX >= imageCanvas.width || pixelY < 0 || pixelY >= imageCanvas.height) {
                continue;
            }
            ...
            if (shouldDraw) {
                const index = (pixelY * imageCanvas.width + pixelX) * 4;
                if (brushMode === 'replace') {
                    if (selectedColor) {
                        data[index] = selectedColor.r;
                        data[index + 1] = selectedColor.g;
                        data[index + 2] = selectedColor.b;
                        data[index + 3] = 255; // Make sure it's fully opaque
                    }
                } else { // erase
                    data[index + 3] = 0;
                }
...
function startPainting(e) {
    if (!isBrushToolActive || !originalImageData) return;
    isPainting = true;
    drawOnCanvas(e);
}
...
function stopPainting() {
    ...
    isPainting = false;
    // Commit the changes to the originalImageData to make them persistent
    originalImageData = ctx.getImageData(0, 0, imageCanvas.width, imageCanvas.height);
}
...
imageCanvas.addEventListener('mouseleave', stopPainting);


brushToolToggle.addEventListener('change', () => {
    isBrushToolActive = brushToolToggle.checked;
    brushControls.classList.toggle('hidden', !isBrushToolActive);
    updateControlStates();
});
...
brushModeReplace.addEventListener('click', () => {
    brushMode = 'replace';
    brushModeReplace.classList.add('active');
    brushModeErase.classList.remove('active');
});
...
brushModeErase.addEventListener('click', () => {
    ...
    brushModeReplace.classList.remove('active');
});
