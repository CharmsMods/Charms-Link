...
var Module=typeof createFFmpegCore!="undefined"?createFFmpegCore:{};var readyPromiseResolve,readyPromiseReject;
...
function preRun(){
if(Module["preRun"]){if(typeof Module["preRun"]==="function")Module["preRun"]=[Module["preRun"]];while(Module["preRun"].length){
...
function postRun(){
if(Module["postRun"]){if(typeof Module["postRun"]==="function")Module["postRun"]=[Module["postRun"]];while(Module["postRun"].length){
...
return MEMFS.createNode(null,"/",{mode:16384|511,dev:0})
},
...
return length
}else if(position===0&&node.usedBytes===0){
...
var root=WORKERFS.createNode(null,"/",{mode:WORKERFS.DIR_MODE,dev:0});
var createdParents={};
...
WORKERFS.createNode(ensureParent(obj["name"]),
base(obj["name"]),
WORKERFS.FILE_MODE,
0,
obj["data"])
...
var stream=FS.createStream({node:node,path:FS.getPath(node),flags:flags,seekable:true,position:0,stream_ops:node.stream_ops},undefined);
if(stream.stream_ops.open){
stream.stream_ops.open(stream)
}
...
if(Module["stdin"]){FS.createDevice("/dev","stdin",Module["stdin"])}else{
FS.symlink("/dev/tty","/dev/stdin")
}
if(Module["stdout"]){FS.createDevice("/dev","stdout",null,Module["stdout"])}else{
FS.symlink("/dev/tty","/dev/stdout")
}
if(Module["stderr"]){FS.createDevice("/dev","stderr",null,Module["stderr"])}else{
...
if(typeof XMLHttpRequest!=="undefined"){
if(!ENVIRONMENT_IS_WORKER){
throw"Cannot do synchronous binary XHRs outside webworkers in modern browsers. Use --embed-file or --preload-file in emcc on the main thread."
}
...
return FS.createNode(null,"/",{mode:16384|511,dev:0})
},
...
var yday=monthDaysCumulative[date.getMonth()]+date.getDate()-1|0;
return yday
}
